...
layui.define(['laytpl', 'laypage', 'layer', 'form','dropdown'], function(exports){
  "use strict";
  ...
  ,thisTable = function(){
    ...
    return {
      ...
      ,addRow: function(rowData){
          if(rowData){
              var oldData = table.cache[that.key] || [], options = {};
      ...
      ,addData: function(data){
        var tableData = table.cache[that.key] || [];
        let newDate = [];
        for (let i= 0; i < tableData.length ; i++) {
            if(tableData[i].length != 0) {
                newDate.push(tableData[i])
            }
        }
        if (data === "") {
          return;
        ...
        that.eachCols(function(i3, item3){
          ...
          var td = ['<td data-field="'+ field +'" '+ function(){
            ...
            ,'<div class="layui-table-cell laytable-cell-'+ function(){ //返回对应的CSS类标识
              ...
              } else if (item3.type === 'radio') {//增加单选
                return '<input type="radio" name="layTableRadio" notitle>';
  ...
  Class.prototype.setCheckData = function(index, checked){
    ...
    that.cacheData[index][options.checkName] = checked;
  };
  ...
  Class.prototype.events = function(){
    ...
    if (options.single) {
      that.layBody.on('click', 'tr', function(e){
        var $tr = $(this);
        var radio = $tr.find('input[name="layTableRadio"]')[0];
        if (radio && !$(radio).is(':checked')) {//未选择，触发单击事件
          ...
          if (data[index]) {
            $.each(data, function (i, item) {
              item[options.checkName] = index == i;
            });
          }
          //触发选择事件
          if (options.select) {
            var data = data ? (data[index] || {}) : {};
            options.select && options.select(data, $tr, index);
          }
        }
      });
    }
    ...
    //单元格事件
    //如果是单选，则改成双击事件
    that.layBody.on(options.single ? 'dbclick' : 'click', 'td', function(){
      var othis = $(this)
